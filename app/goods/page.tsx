...
async function getProps() {
  try {
    const files = fs.readdirSync('public/goods')

    const posts: Post[] = files.map((fileName) => {
      const slug = fileName.replace('.md', '')
      const readFile = fs.readFileSync(`public/goods/${fileName}`, 'utf-8')
        ...
        posts.map(({ slug, frontmatter }) => (
          <div key={ slug } className="border border-gray-200 m-2 rounded-xl shadow-lg overflow-hidden flex flex-col">
            <Link href={ `/goods/${slug}` }>
              ...
              <h3 className='text-right pr-4'>{frontmatter?.price}</h3>
              <h3 className='italic pl-6'>{frontmatter?.description}</h3>
